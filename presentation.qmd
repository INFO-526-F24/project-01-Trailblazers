...
age_group_count <- data.frame(
  Age_Group = c("0-18", "19-35", "36-50", "51-65", "65+"),
  Count = c(886, 13519, 12167, 12298, 16096)
)

# Create a bar plot
ggplot(data = age_group_count, aes(x = Age_Group, y = Count, fill = Age_Group)) +
...
ggplot(age_group_revenue, aes(x = "", y = Total_Revenue_Million, fill = Age_Group)) +  # Create pie chart with filled segments based on Age Group
  ...
  theme_void() + #Removning x on x-axis
  theme(plot.title = element_text(hjust = 0.5, vjust = - 88)) +  # Center the title at bottom of the plot
